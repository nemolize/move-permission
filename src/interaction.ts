...
export const formatBrokenLayers = (layers: Layer[]): string[] =>
  layers
    ...
    .map(
      (layer) => `[${layer.name}] ${layer.path} ${layer.error ?? ""} (skipped)`,
    );
...
export const formatEntries = (
  ...
  return entries.map((entry, index) => {
    ...
    return `${index + 1}. ${includeLayer ? `[${entry.layer}] ` : ""}permissions.${entry.field} ${JSON.stringify(entry.value)}${duplicateNotice}`;
  });
};
...
export const promptForMoves = async (
  ...
  if (sourceScopes.length === 0) {
    formatEntries(allEntries).forEach(writeLine);
    return undefined;
  }
  writeLine(
    "From scopes: " +
      sourceScopes
        .map(
          (scope, index) =>
            `${index + 1}. ${scope.layer} (${scope.entries.length} entries)`,
        )
        .join("  "),
  );
  ...
  formatEntries(entries, allEntries, false).forEach(writeLine);
