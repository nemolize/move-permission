...
it("deleting removes only the selected permission field value", () => {
  ...
  expect(planned[0].settings.permissions).toEqual({ allow: [], ask: ["Read(*)"] });
});
...
it("does not mistake an unrelated allow key for permissions.allow", () => {
  ...
  const planned = applyMoves([original], [{ source: { layer: "user", field: "allow", value: "old" } }]);
  writeLayersAtomically(changedLayers([original], planned));
  expect(readFileSync(path, "utf8")).toBe(source.replace('"permissions": { "allow": ["old"] }', '"permissions": { "allow": [] }'));
});
...
it("a move creates a missing destination only when it receives an entry", () => {
  ...
  const planned = applyMoves([source, destination], [{ source: { layer: "project-local", field: "allow", value: "Bash(a)" }, destination: "user" }]);
  expect(planned[1].settings).toEqual({ permissions: { allow: ["Bash(a)"] } });
...
it("lists values from all supported permission fields", () => {
  ...
  expect(entries.map((entry) => entry.field)).toEqual(["allow", "ask", "deny"]);
});
});
